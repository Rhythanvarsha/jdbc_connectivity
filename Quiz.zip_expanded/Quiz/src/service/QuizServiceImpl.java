...
public class QuizServiceImpl implements QuizService {
    ...
    public void addQuestion(Question question) {
        try (Connection connection = DatabaseConnection.getConnection()) {
            //  Check if the quiz with the provided quiz_id exists in the Quiz table
            String checkQuizSql = "SELECT quiz_id FROM Quiz WHERE quiz_id = ?";
            PreparedStatement checkQuizStatement = connection.prepareStatement(checkQuizSql);
            checkQuizStatement.setInt(1, question.getQuizId());
            ResultSet quizResult = checkQuizStatement.executeQuery();

            if (!quizResult.next()) {
                // Quiz with the specified quiz_id does not exist
                System.out.println("Error: Quiz with ID " + question.getQuizId() + " does not exist. Please add the quiz first.");
                return; // Exit the method to prevent the SQL exception
            }

            //  If quiz exists, proceed to add the qestion
            String insertQuestionSql = "INSERT INTO Questions (quiz_id, question_text, question_type, correct_answer) VALUES (?, ?, ?, ?)";
            PreparedStatement statement = connection.prepareStatement(insertQuestionSql);
